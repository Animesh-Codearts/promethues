 ...
 * @return array
 */
function promethues_body_classes( $classes ) {
...
add_filter( 'body_class', 'promethues_body_classes' );
 ...
 * @return array
 */
function promethues_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'promethues_post_classes', 10, 3 );
 ...
 * @return void
 */
function promethues_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'promethues_pingback_header' );
 ...
 * @return void
 */
function promethues_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'promethues_supports_js' );
 ...
 * @return array
 */
function promethues_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'promethues_comment_form_defaults' );
 ...
 * @return bool
 */
function promethues_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'promethues_can_show_post_thumbnail',
 ...
 * @return int
 */
function promethues_get_avatar_size() {
	return 60;
}
 ...
 */
function promethues_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Post title. Only visible to screen readers. */
		esc_html__( 'Continue reading %s', 'promethues' ),
 ...
 */
function promethues_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . promethues_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'promethues_continue_reading_link_excerpt' );
 ...
 */
function promethues_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . promethues_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'promethues_continue_reading_link' );

if ( ! function_exists( 'promethues_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function promethues_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'promethues' ) : $title;
	}
}
add_filter( 'the_title', 'promethues_post_title' );
 ...
 * @return string
 */
function promethues_get_icon_svg( $group, $icon, $size = 24 ) {
	return promethues_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function promethues_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? promethues_get_icon_svg( 'ui', 'arrow_right' ) : promethues_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? promethues_get_icon_svg( 'ui', 'arrow_left' ) : promethues_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'promethues_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function promethues_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'promethues_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'promethues_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'promethues_generate_css' ) ) {
	...
	return promethues_generate_css( // @phpstan-ignore-line.
 ...
 */
function promethues_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 * @since promethues 1.4 Corrected parameter name for `$output`,
 ...
 */
function promethues_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'promethues' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'promethues' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'promethues' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'promethues' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'promethues_password_form', 10, 2 );
 ...
 */
function promethues_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'promethues_get_attachment_image_attributes', 10, 3 );
